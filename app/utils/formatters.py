from __future__ import annotations

from collections import defaultdict
from datetime import date, datetime
from decimal import Decimal
from typing import Iterable

from app.models import Connection, Draft, HourLog, SatisfactionScore, Task


def format_hours(value) -> str:
    return f'{Decimal(value or 0):.2f}'.rstrip('0').rstrip('.')


def render_timesheet_table(va_name: str, week_start: date, logs: Iterable[HourLog], rate: Decimal | None = None) -> str:
    day_map: dict[date, list[HourLog]] = defaultdict(list)
    for log in logs:
        day_map[log.work_date].append(log)
    week_end = week_start.fromordinal(week_start.toordinal() + 6)
    lines = [
        '📋 BeeSmartVA — Weekly Timesheet',
        '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
        f'👤 VA:   {va_name}',
        f'📅 Week: {week_start.isoformat()} → {week_end.isoformat()}',
    ]
    if rate is not None:
        lines.append(f'💰 Rate: ${format_hours(rate)}/hr')
    lines.extend(['━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', ''])
    total = Decimal('0')
    day_emojis = ['🟦', '🟦', '🟦', '🟦', '🟦', '🟧', '🟧']  # Mon-Fri blue, Sat-Sun orange
    for offset in range(7):
        current_day = week_start.fromordinal(week_start.toordinal() + offset)
        entries = day_map.get(current_day, [])
        if not entries:
            continue
        total_day = sum((Decimal(str(entry.hours)) for entry in entries), Decimal('0'))
        note = ' | '.join(filter(None, [entry.note for entry in entries]))
        emoji = day_emojis[offset]
        lines.append(f'{emoji} {current_day.strftime("%A"):<10} {format_hours(total_day):>5}h   {note[:70]}')
        total += total_day
    lines.append('')
    lines.append('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    est = f'\n💵 Estimated: ${format_hours(total * rate)}' if rate is not None else ''
    lines.append(f'⏱ Total hours: {format_hours(total)}h{est}')
    return '\n'.join(lines)


def render_myweek(logs: Iterable[HourLog]) -> str:
    total = Decimal('0')
    lines = ['⏱ Your Week So Far', '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', '']
    for log in sorted(logs, key=lambda x: (x.work_date, x.id)):
        total += Decimal(str(log.hours))
        lines.append(f'📅 {log.work_date.isoformat()}  {format_hours(log.hours)}h  —  {log.note or "—"}')
    lines.append('')
    lines.append('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    lines.append(f'⏱ Total: {format_hours(total)}h')
    return '\n'.join(lines)


def render_task_list(tasks: Iterable[Task], user_map: dict[int, str] | None = None) -> str:
    tasks = list(tasks)
    if not tasks:
        return '✅ No open tasks in this group. All clear!'
    user_map = user_map or {}
    lines = [f'📋 Open Tasks ({len(tasks)})', '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', '']
    now = datetime.utcnow()
    status_icons = {'open': '🔵', 'done': '✅', 'flagged': '🚩'}
    for task in tasks:
        age_hours = int((now - task.created_at).total_seconds() // 3600)
        assignee = user_map.get(task.assigned_to, 'unassigned') if task.assigned_to else 'unassigned'
        flag = f' ⚠️ {task.flag_reason.value}' if task.flag_reason else ''
        icon = status_icons.get(task.status.value, '🔵')
        lines.append(f'{icon} #{task.id} — {task.description}')
        lines.append(f'   👤 {assignee} · {age_hours}h old{flag}')
        lines.append('')
    return '\n'.join(lines).rstrip()


def render_stats(open_tasks: int, flagged_tasks: int, total_hours: Decimal, pending_timesheets: int, pending_drafts: int = 0, pending_followups: int = 0) -> str:
    return (
        '📊 Quick Stats\n'
        '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n'
        f'📋 Open tasks:          {open_tasks}\n'
        f'🚩 Flagged tasks:       {flagged_tasks}\n'
        f'⏱ Hours this week:     {format_hours(total_hours)}h\n'
        f'📤 Pending timesheets:  {pending_timesheets}\n'
        f'📝 Pending drafts:      {pending_drafts}\n'
        f'🔗 Pending follow-ups:  {pending_followups}'
    )


def render_connections(items: Iterable[Connection]) -> str:
    items = list(items)
    if not items:
        return '✅ No pending follow-ups — you\'re all caught up!'
    lines = [f'🔗 Follow-up Queue ({len(items)})', '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', '']
    status_icons = {'pending': '⏳', 'replied': '💬', 'booked': '📅', 'no_response': '🔕', 'done': '✅'}
    for item in items:
        due = item.followup_due_at.strftime('%Y-%m-%d %H:%M') if item.followup_due_at else '—'
        icon = status_icons.get(item.status.value, '🔵')
        lines.append(f'{icon} {item.prospect_name}')
        lines.append(f'   📱 {item.platform} · 🕐 Due: {due}')
        lines.append('')
    return '\n'.join(lines).rstrip()


def render_drafts(items: Iterable[Draft]) -> str:
    items = list(items)
    if not items:
        return '📝 No drafts found.'
    lines = [f'📝 Drafts ({len(items)})', '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', '']
    status_icons = {'pending': '⏳', 'approved': '✅', 'revision_requested': '✏️', 'posted': '🚀'}
    for draft in items:
        icon = status_icons.get(draft.status.value, '📄')
        lines.append(f'{icon} {draft.draft_code} — {draft.platform}')
        lines.append(f'   🕐 {draft.submitted_at:%Y-%m-%d %H:%M} · {draft.status.value}')
        lines.append('')
    return '\n'.join(lines).rstrip()


def render_scores(items: Iterable[SatisfactionScore]) -> str:
    items = list(items)
    if not items:
        return '⭐ No satisfaction scores recorded yet.'
    score_emojis = {1: '😕', 2: '😐', 3: '🙂', 4: '😊', 5: '🌟'}
    lines = [f'⭐ Satisfaction Scores ({len(items)})', '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', '']
    for score in items:
        emoji = score_emojis.get(score.score, '⭐')
        comment = f'\n   💬 "{score.comment}"' if score.comment else ''
        lines.append(f'{emoji} {score.period_label}  —  {score.score}/5')
        lines.append(f'   📌 {score.trigger_type.value}{comment}')
        lines.append('')
    return '\n'.join(lines).rstrip()
